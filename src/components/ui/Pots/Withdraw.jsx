...
const Withdraw = ({pots, pot, setPotToWithdraw, UPDATE}) => {
  const [amountToWithdraw, setAmountToWithdraw] = useState(0);
  const [customMessage, setCustomMessage] = useState('Amount withdrawn successfully!');
  ...
  const removedPercent = ((amountToWithdraw / pot.target) * 100)
  ...
  const handleWithdrawMoney = async (potID) => {
    if (amountToWithdraw <= 0) {
      setCustomMessage('Please enter a valid amount');
      setCustomMessageActive(true);
      setTimeout(() => setCustomMessageActive(false), 1000);
      return;
    }
    if (amountToWithdraw > pot.total) {
      setCustomMessage('Insufficient funds in the pot');
      setCustomMessageActive(true);
      setTimeout(() => setCustomMessageActive(false), 1000);
      return;
    }
    ...
    if (response.ok) {
      UPDATE()
      setCustomMessage('Amount withdrawn successfully!');
      setCustomMessageActive(true);
      setSuccess(true);
      setTimeout(() => {
        setCustomMessageActive(false);
        setPotToWithdraw(null); // Close the modal
      }, 1000);
     
    } else {
      console.error('Error withdrawing money from pot:', updatedPot);
    }
  };
    ...
    <div className='absolute top-0 left-0 z-50 w-full h-full bg-[#00000025] overflow-y-hidden'>
      <div className='absolute abs_center w-[520px] max-sm:w-[90%] bg-white p-[25px] rounded-[10px] flex flex-col gap-[25px] shadow-sm'>
        ...
        <div id='HEAD_ROW' className='flex justify-between items-center '>
          <h1 className='txt5'>Withdraw From {pot.name}</h1>
          ...
          <div id='NEW_NAME' className='w-full flex flex-col gap-[5px]'>
            ...
            <div className="flex flex-col gap-[10px] items-center justify-between">
                <div id="PROGRESS_BAR" className=" relative overflow-x-hidden w-full rounded-full bg-[#d0d0d0] h-[7.5px]">
                   <div id='DEFAULT_PERCENT' className={`z-20 absolute top-0 left-0 rounded-full h-full`} style={{ width: `${percent}%`, backgroundColor: removedPercent? 'black' : `${themeColor}`}}> </div>
                   <div id='REMOVE_PROGRESS' className={`${!removedPercent > 0 && 'hidden'} z-50 absolute top-0 bg-RED border-l-[3px] rounded-full ${removedPercent && 'rounded-l-none' } h-full transition-all ease-in-out duration-300`} 
                   style={{
                    width: `${removedPercent}%`,
                    right: `${'100' - percent}%`}}></div>
                </div>
                
                <div id="PERCENT__TARGET" className="w-full flex justify-between">
                  <p className="font-sans text-[13px] text-[#6c6c6c]">
                    <span className={`font-[700] ${removedPercent > 0 && 'text-RED'}`}>{removedPercent > 0? 
                     (parseFloat(formattedPercent) - parseFloat(removedPercent)).toFixed(2)
                      : 
                     formattedPercent}%
                    </span>
                  </p>
        ...
        <button onClick={() => handleWithdrawMoney(pot._id)} id='SAVE' className='w-full gray1 text-[#fff] p-[17px] rounded-[10px] font-sans font-[550] tracking-[0.5px] hover:text-[17px] transition1'>
          Confirm Withdrawal
        </button>
                      
        <div className={`absolute z-10 rounded left-[50%] translate-x-[-50%] ${customMessageActive ? 'bottom-[-50px] opacity-100' : 'bottom-[0px] opacity-0'} bg-white text-black border w-[80%] h-[30px] flex items-center justify-center transition2`}>
          <p className='font-sans font-[700]'>{customMessage} {success ? <i className='text-GREEN ml-[5px] fa-solid fa-circle-check'></i> : <i className='text-RED fa-solid fa-circle-exclamation'></i>}</p>
        </div>
      </div>
    </div>
  )
